...
class Promociones_detallesController extends Controller
{
    public function index(){
        ...
        if(Auth::check()){
            if($role){
                $promociones_vigentes = $this->promocionesVigentes();
    ...
    public function promocionesVigentes(){
     ...
     $promocionesVigentes = Promocion_detalles::with(['productos' => function ($query) {
        $query->select('codigo', 'nombre'); // Seleccionar solo los campos necesarios de productos
    }])
    ->where('fecha_inicio', '<=', $fechaActual) // La promoción ya comenzó
    ->where('fecha_final', '>=', $fechaActual) // La promoción aún no ha terminado
    ->get(); // Obtener las promociones vigentes con los productos
    ...
    public Function productoTienePromocionVigente(string $productoCodigo){
        $promociones = $this->promocionesVigentes();
        foreach($promociones as $promocion){
                ...
                if($promocion_detalle_producto){
                   $porcentajeDescuento = $promocion_detalle_producto->pivot->porcentaje; 
                   if ($porcentajeDescuento > 0) {
                       return [true, 'con_descuento', $porcentajeDescuento];
                   }
                }
            }
